          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <p className="font-mono text-[11px] uppercase tracking-widest text-white/60">Fleet Live Tracking</p>
              ...
              <h1 className="mt-2 text-[28px] font-semibold tracking-tight text-white">Global Fleet Map</h1>
              <p className="mt-1 text-[13px] text-white/60">
                Real-time positions of all {aircraft.length} aircraft in our fleet
              </p>
            </div>
            <div className="flex items-center gap-2">
              <div className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-3 py-1.5 ring-1 ring-emerald-400/20">
                <Plane className="h-3.5 w-3.5 text-emerald-300" strokeWidth={2} />
                <span className="text-[12px] font-medium text-emerald-300">{inFlight} in flight</span>
              </div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white/[0.04] px-3 py-1.5 ring-1 ring-white/10">
                <MapPin className="h-3.5 w-3.5 text-white/70" strokeWidth={2} />
                <span className="text-[12px] font-medium text-white/80">{onGround} on ground</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-6 lg:flex-row">
          ...
          <div className="flex-1">
            <div className="relative aspect-[16/10] overflow-hidden rounded-2xl border border-white/[0.08] bg-gradient-to-br from-neutral-950 via-black to-neutral-950 backdrop-blur-xl">
              {/* Grid */}
              ...
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex items-center gap-3 rounded-full border border-white/[0.08] bg-black/60 px-6 py-3 backdrop-blur-md">
                    <RefreshCw className="h-4 w-4 animate-spin text-white/70" strokeWidth={1.75} />
                    <span className="text-[12px] text-white/70">Loading fleet positions...</span>
                  </div>
                </div>
              )}

              {/* Legend */}
              <div className="absolute bottom-3 left-3 flex items-center gap-4 rounded-lg border border-white/[0.08] bg-black/60 px-3 py-2 backdrop-blur-md">
                <div className="flex items-center gap-1.5">
                  <div className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
                  <span className="font-mono text-[10px] uppercase tracking-widest text-white/60">In Flight</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="h-1.5 w-1.5 rounded-full bg-white/40" />
                  <span className="font-mono text-[10px] uppercase tracking-widest text-white/60">On Ground</span>
                </div>
              </div>

              <div className="absolute bottom-3 right-3 rounded border border-white/10 bg-black/60 px-2 py-0.5 font-mono text-[9px] uppercase tracking-widest text-white/40 backdrop-blur-md">
            ...
            {selected && (
              <div className="mt-4 rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5 backdrop-blur-xl">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-[15px] font-semibold tracking-tight text-white">{selected.jet.name}</h3>
                      <p className="text-[12px] text-white/50">{selected.jet.manufacturer} · {selected.registration}</p>
                    </div>
                  </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4 border-t border-white/[0.08] pt-4 sm:grid-cols-4">
                ...
                <div className="mt-4 flex gap-3">
                  <Link
                    ...
                    className="flex-1 rounded-full border border-white/15 bg-white/[0.03] px-4 py-2 text-center text-[12px] font-medium text-white transition-colors hover:border-white/30 hover:bg-white/[0.06]"
                  >
                    View Details
                  </Link>
                  <Link
                    href={`/booking?jet=${selected.jet.id}`}
                    className="flex-1 rounded-full bg-white px-4 py-2 text-center text-[12px] font-medium text-black transition-colors hover:bg-white/90"
                  >
                    Book Now
                  </Link>
                </div>
          ...
          <div className="w-full shrink-0 lg:w-80">
            ...
            <div className="mb-4 space-y-3">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-white/40" strokeWidth={2} />
                <input
                  type="text"
                  placeholder="Search aircraft..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full rounded-lg border border-white/10 bg-white/[0.03] py-2.5 pl-9 pr-4 text-[13px] text-white placeholder:text-white/30 outline-none focus:border-white/40"
                />
              </div>
                ...
                {(["all", "flying", "ground"] as const).map((f) => (
                  <button
                    ...
                    className={cn(
                      ...
                      filter === f
                        ? "bg-white text-black"
                        : "border border-white/15 bg-white/[0.03] text-white/70 hover:border-white/30 hover:bg-white/[0.06]"
                    )}
                  >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <p className="truncate text-[13px] font-semibold text-white">{a.jet.name}</p>
                        <div
                          className={cn(
                            "ml-2 h-1.5 w-1.5 shrink-0 rounded-full",
                            a.onGround ? "bg-white/40" : "animate-pulse bg-emerald-400"
                          )}
                        />
                      </div>
