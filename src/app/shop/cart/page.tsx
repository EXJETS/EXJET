      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        {items.length === 0 ? (
          <div className="bg-white rounded-3xl border border-gray-100 p-20 text-center">
            ...
            <Link
              href="/shop"
              className="inline-flex items-center gap-2 px-8 py-3.5 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-colors"
            >
              Continue Shopping
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {items.map(({ product, quantity }) => {
                ...
                return (
                  ...
                  >
                    <Link href={`/shop/products/${product.slug}`} className="relative h-28 w-28 rounded-xl overflow-hidden bg-gray-50 shrink-0">
                      <Image
                        src={product.image}
                        alt={product.name}
                        fill
              ...
              <Link
                href="/shop"
                className="inline-flex items-center gap-2 text-sm text-indigo-600 hover:text-indigo-800 font-medium transition-colors mt-2"
              >
                ← Continue Shopping
              </Link>
            </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl border border-gray-100 p-6 sticky top-28">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:outline-none focus:border-indigo-400 transition-colors"
                      />
                    </div>
                    <button className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-xl transition-colors">
                      Apply
                    </button>
                  </div>
                </div>
                ...
                <Link
                  href="/shop/checkout"
                  className="mt-6 block w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white text-center font-semibold rounded-xl transition-colors shadow-lg shadow-indigo-200"
                >
                ...
                <div className="mt-4 flex items-center justify-center gap-4">
                  {["visa", "mastercard", "amex", "paypal"].map((card) => (
                    <div
                      key={card}
                      className="h-6 px-2 bg-gray-100 rounded text-[10px] font-bold text-gray-500 flex items-center uppercase"
                    >
                      {card}
                    </div>
                  ))}
