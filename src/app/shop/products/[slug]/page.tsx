...
import { products, getProductBySlug } from "@/data/shop-products";
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-1.5 text-xs text-gray-500">
            ...
            <Link href={`/shop/category/${product.categorySlug}`} className="hover:text-indigo-600 transition-colors">
              {product.category}
            </Link>
            <ChevronRight className="h-3 w-3" />
            ...
            {product.images.length > 1 && (
              <div className="flex gap-3">
                {product.images.map((img, i) => (
                  <div
                    key={i}
                    className="relative h-20 w-20 rounded-xl overflow-hidden bg-gray-50 border-2 border-transparent hover:border-indigo-400 transition-colors cursor-pointer"
                  >
          ...
          <div className="flex flex-col gap-5">
            <div>
              <Link
                ...
                className="text-xs font-semibold text-indigo-600 uppercase tracking-wider hover:text-indigo-800"
              >
                {product.category}
              </Link>
...
function getBadgeClass(badge: string) {
  const map: Record<string, string> = {
    "Best Seller": "absolute top-4 left-4 px-3 py-1.5 bg-amber-500 text-white text-sm font-semibold rounded-full",
    Hot: "absolute top-4 left-4 px-3 py-1.5 bg-red-500 text-white text-sm font-semibold rounded-full",
    New: "absolute top-4 left-4 px-3 py-1.5 bg-indigo-600 text-white text-sm font-semibold rounded-full",
    Sale: "absolute top-4 left-4 px-3 py-1.5 bg-emerald-600 text-white text-sm font-semibold rounded-full",
    Limited: "absolute top-4 left-4 px-3 py-1.5 bg-purple-600 text-white text-sm font-semibold rounded-full",
  };
  return map[badge] ?? "";
}
