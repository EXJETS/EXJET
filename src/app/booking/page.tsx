...
function BookingContent() {
  ...
  const inputCls =
    "w-full rounded-lg border border-[var(--color-hairline)] bg-[var(--color-ivory)] py-3 pl-10 pr-4 text-[13px] text-[var(--color-ink)] placeholder:text-[var(--color-subtle)] outline-none focus:border-[var(--color-champagne)]";
  const dateInputCls =
    "w-full rounded-lg border border-[var(--color-hairline)] bg-[var(--color-ivory)] py-3 pl-10 pr-3 text-[13px] text-[var(--color-ink)] placeholder:text-[var(--color-subtle)] outline-none focus:border-[var(--color-champagne)]";
  const labelCls = "mb-1.5 block font-mono text-[10px] uppercase tracking-widest text-[var(--color-muted)]";
  ...
  if (!jet) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-white px-4 text-[var(--color-ink)]">
        ...
        <h2 className="text-[20px] font-semibold tracking-tight text-[var(--color-ink)]">No aircraft selected</h2>
        <p className="mt-2 text-[13px] text-[var(--color-muted)]">Please select a jet from our fleet first.</p>
        <Link
          href="/search"
          className="mt-6 rounded-full bg-[var(--color-ink)] px-6 py-3 text-[13px] font-medium text-white transition-colors hover:bg-[var(--color-ink-soft)]"
        >
          Browse Jets
        </Link>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 flex flex-col gap-8 lg:flex-row">
          ...
          <div className="flex-1">
            {/* Selected Jet */}
            <div className="mb-6 flex items-center gap-4 rounded-2xl border border-[var(--color-hairline)] bg-white p-4 backdrop-blur-xl">
              <div className="flex h-12 w-16 items-center justify-center rounded-lg border border-[var(--color-hairline)] bg-[var(--color-ivory-deep)]">
                <Plane className="h-5 w-5 text-[var(--color-ink-soft)]" strokeWidth={1.75} />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="truncate text-[14px] font-semibold tracking-tight text-[var(--color-ink)]">{jet.name}</h3>
              ...
              <p className="text-[16px] font-semibold tracking-tight text-[var(--color-ink)]">
                ...
                <span className="text-[12px] font-normal text-[var(--color-muted)]">/hr</span>
              </p>
            </div>

            <div className="space-y-5 rounded-2xl border border-[var(--color-hairline)] bg-white p-6 backdrop-blur-xl">
              <p className="font-mono text-[11px] uppercase tracking-widest text-[var(--color-muted)]">Trip Details</p>
              ...
              <div className="relative">
                ...
                {showDepDropdown && filterAirports(depSearch).length > 0 && (
                  <div className="absolute left-0 right-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-xl border border-[var(--color-hairline)] bg-[var(--color-ink)] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.18)] backdrop-blur-xl">
                    {filterAirports(depSearch).map((a) => (
                      ...
                      >
                        ...
                        <div>
                          <span className="text-[13px] font-semibold text-[var(--color-ink)]">{a.code}</span>
                          <span className="ml-2 text-[13px] text-[var(--color-muted)]">{a.city}, {a.country}</span>
                          <p className="text-[11px] text-[var(--color-subtle)]">{a.name}</p>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--color-subtle)]" strokeWidth={1.75} />
                    <input
                      type="date"
                      min={today}
                      value={departureDate}
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--color-hairline)] bg-[var(--color-ivory)] text-[var(--color-ink)] transition-colors hover:border-[var(--color-hairline-strong)] hover:bg-[var(--color-ivory)] disabled:opacity-40"
                    disabled={passengerCount <= 1}
                  >
                    −
                  </button>
                  <div className="flex items-center gap-2">
                    <Users className="h-4 w-4 text-[var(--color-subtle)]" strokeWidth={1.75} />
                    <span className="w-8 text-center text-[18px] font-semibold text-[var(--color-ink)]">{passengerCount}</span>
                  </div>
                  <button
                  ...
                  <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--color-subtle)]">Max {jet.passengers}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-between">
              <Link
                href={`/jets/${jet.id}`}
                className="rounded-full border border-[var(--color-hairline)] bg-[var(--color-ivory)] px-5 py-2.5 text-[12px] font-medium text-[var(--color-ink)] transition-colors hover:border-[var(--color-hairline-strong)] hover:bg-[var(--color-ivory)]"
              >
                Back to Jet
              </Link>
              <button
    ...
    <Suspense
      fallback={
        <div className="flex min-h-screen items-center justify-center bg-white">
          <Plane className="h-6 w-6 animate-pulse text-[var(--color-subtle)]" strokeWidth={1.75} />
        </div>
      }
    >
      <BookingContent />
